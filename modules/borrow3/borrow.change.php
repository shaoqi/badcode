...
class borrowChangeClass{
	function GetChangeList($data){
		...
		$_select = "p0.*,p0.web_account as web_buy,p1.recover_times,p1.id as tid,p1.recover_account_wait,p1.recover_account_capital_wait,p1.user_id as tuser,p1.recover_account_interest_wait,p2.username,p3.name as borrow_name,p3.user_id as borrow_user_id,p3.borrow_period,p3.borrow_apr,p3.borrow_nid,p3.borrow_type,p3.borrow_style,p4.username as buy_username";
	...
	function ActionChange($data){
		global $mysql;
		$sql = "select p1.*,p2.username as name,p3.user_id as borrow_user_id,p3.name as borrow_name,p4.buy_userid from `{borrow_tender}`as p1 left join `{users}` as p2 on p1.user_id=p1.user_id left join `{borrow}` as p3 on p3.borrow_nid=p1.borrow_nid left join `{borrow_change}` as p4 on p4.tender_id=p1.id where p1.user_id='{$data['user_id']}' and p1.id='{$data['id']}'";
		$result = $mysql->db_fetch_array($sql);
		$tender_userid = $result['user_id'];
		...
		$mysql->db_query($sql);
		$tendersql = "select 1 from `{borrow_change}` where user_id='{$data['user_id']}' and tender_id='{$data['id']}'";
		...
		if ($tender_result==false){
			...
			$sql = "update `{borrow_change}` set user_id='{$data['user_id']}',tender_id='{$data['id']}',status=2,account='{$data['account']}',remark='{$data['remark']}',valid_day='{$data['valid_day']}' where user_id='{$data['user_id']}' and tender_id='{$data['id']}'";
		}
		$mysql->db_query($sql);	
	...
	function BuyChange($data){
		...
		$change_fee = round($tender_result['recover_account_interest_wait']*$_G['system']['con_change_fee'],2);//管理费用
