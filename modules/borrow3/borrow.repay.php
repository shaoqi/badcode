...
class borrowrepayClass
{
	...
	function RepayStep0($data){
		...
		$late_interest = round($repay_account/100*0.8*$late_days,2);
		...
		if ($repay_period+1 == $borrow_period){
			if ($borrow_frost_account>0){
				...
				$log_info["to_userid"] = 0;//付给谁
				$log_info["remark"] = "借款标[{$borrow_url}]保证金返还";
				accountClass::AddLog($log_info);
			}
			borrowClass::UpdateBorrowCount(array("user_id"=>$borrow_userid,"borrow_repay_all_times"=>1));
		}
	...
	function RepayStep1($data){
		...
		$repay_period = $repay_result["repay_period"];
		$repay_web = $repay_result["repay_web"];
		$repay_five = $repay_result["repay_web_five_status"];
		$repay_ten = $repay_result["repay_web_ten_status"];
		$repay_vouch = $repay_result["repay_vouch"];
		$repay_period = $repay_result["repay_period"];
		$repay_account = $repay_result["repay_account"];//还款总额
		$repay_capital = $repay_result["repay_capital"];//还款本金
		...
		if ($re_time!=0){
			if ($re_time<0){
				...
				if ($re_time<=30 && $re_time>=1){
					$borrow_credit_nid = "borrow_repay";							
				}
			}
		}
		...
		//第十三步，用户正常还款
		if ($repay_web!=1 && $repay_vouch!=1 && $repay_five!=1 && $repay_ten!=1){
			...
			//利息超过7天，则逾期利息为0
			$late_days = $late['late_days'];
			$late_interest = round($value['recover_account']*0.004*$late['late_days']/2,2);//罚金
			$late_reminder = round($value['recover_account']*0.008*$late['late_days']/2,2);//滞纳金
			
			$sql = "update  `{borrow_recover}` set recover_yestime='".time()."',recover_account_yes = recover_account ,recover_capital_yes = recover_capital ,recover_interest_yes = recover_interest ,status=1,recover_status=1,late_days={$late['late_days']},late_interest={$late_interest} where id = '{$recover_result['id']}'";
			$mysql->db_query($sql);
			
			if ($recover_result['change_status']==1){
				$recover_result['user_id'] = $recover_result['change_userid'];
			}
			//添加投资者积分
			...
			if($late_interest>0){
				...
				$log_info["remark"] = "用户{$borrow_username}对[{$borrow_url}]还款，逾期7天内罚息收入";
				accountClass::AddLog($log_info);
			}
			...
			$remind['title'] = "您的投资标[{$borrow_url}]还款";
			$remind['content'] = "您的投资标[{$borrow_url}]在".date("Y-m-d",time())."还款";
			remindClass::sendRemind($remind);
			
			if ($late_days>0 && $late_days<=7){
				$log_info["user_id"] = $recover_result['user_id'];//操作用户id
				...
				$log_info["remark"] = "客户（{$borrow_username}）对[{$borrow_url}]借款逾期7天还款的罚金";
				accountClass::AddLog($log_info);
	...
	function RepayStep4($data){
		...
		if ($borrow_type==5){
			...
			borrowClass::AddAmountLog($_data);
			return -1;
		}elseif ($borrow_type==2 ){
			//抵押额度的返还
			$_data["user_id"] = $borrow_userid;
			$_data["amount_type"] = "diya_borrow";
			$_data["type"] = "borrrow_repay";
			$_data["oprate"] = "return";
			$_data["nid"] = "borrrow_diya_".$borrow_userid."_".$borrow_nid."_".$repay_id;
			$_data["account"] = $repay_capital;
			$_data["remark"] = "借款抵押标[{$borrow_url}]成功还款，额度增加";
			borrowClass::AddAmountLog($_data);
			return -1;
		}
