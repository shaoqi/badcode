<ul class="nav3"> 
<li><a href="{$_A.query_url}" {if $_A.query_type==""}style="color:red"{/if}>用户积分</a></li> 
<li><a href="{$_A.query_url}/log" {if $_A.query_type=="log"}style="color:red"{/if}>积分记录</a></li> 
<li><a href="{$_A.query_url}/rank" {if $_A.query_type=="rank"}style="color:red"{/if}>积分等级</a></li> 
<li><a href="{$_A.query_url}/type" {if $_A.query_type=="type"}style="color:red"{/if}>积分类型</a></li> 
<li><a href="{$_A.query_url}/class" {if $_A.query_type=="class"}style="color:red"{/if}>积分分类</a></li> 
</ul> 
...
<div class="module_add">
	<div class="module_title"><strong>用户积分</strong><span style="float:right">
			用户名：<input type="text" name="username" id="username" value="{$magic.request.username}"/> <input type="button" value="搜索" / onclick="sousuo()"></span></div>
</div>
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	...
	{ list module="credit" function="GetList" var="loop" username=request epage="20"}
	...
	<tr {if $key%2==1}class="tr2"{/if}>
		...
		<td >{$Cvar.tender_credit|default:0}</td>
		<td >{$Cvar.borrow_credit|default:0}</td>
		<td >{$Cvar.att_credit|default:0}</td>
		{/articles}
...
<div class="module_add">
	...
	<div class="module_border">
		...
		<div class="c">
		{input type="radio" name="status" value="1|启用,0|关闭" checked="$_A.credit_type_result.status"}
		</div>
	</div>
	
    
	<div class="module_border">
		<div class="l">积分类型：</div>
		<div class="c">
        <input type="radio" name="credit_type" value="0" {if $_A.credit_type_result.credit_type=="0" || $_A.credit_type_result.credit_type==""} checked=""{/if} onclick="$('#id_value').show();$('#id_value_scale').hide();"/>积分值
        <input  type="radio" name="credit_type" value="1" {if $_A.credit_type_result.credit_type=="1" } checked=""{/if}  onclick="$('#id_value').hide();$('#id_value_scale').show();"/>比例
	
		</div>
	</div>
	<div class="module_border" id="id_value" {if $_A.credit_type_result.credit_type!=0} style="display:none"{/if}>
		<div class="l">积分值：</div>
		<div class="c">
	...
	<div class="module_border"  id="id_value_scale"  {if $_A.credit_type_result.credit_type!="1"} style="display:none"{/if}>
		<div class="l">比例：</div>
		<div class="c">
			<input type="text" name="value_scale"  class="input_border" value="{ $_A.credit_type_result.value_scale|default:0}" size="3" onkeyup="value=value.replace(/[^0-9.]/g,'')"/> %
		</div>
	</div>
	...
	<div style="float:right; width:67%; text-align:left">
	<div class="module_add">
	<div class="module_title"><strong>积分类型列表</strong><span style="float:right">
				名称：<input type="text" name="name" id="name" value="{$magic.request.name|urldecode}"/>  标识名：<input type="text" name="nid" id="nid" value="{$magic.request.nid}" onkeyup="value=value.replace(/[^a-zA-Z_]/g,'')"/>   <input type="button" value="{$MsgInfo.users_name_sousuo}" / onclick="sousuo()"></span></div>
	</div>
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	<tr >
		<td width="" class="main_td">ID</td>
	...
	{ list module="credit" function="GetTypeList" var="loop" epage=20 name=request nid=request }
	{foreach from="$loop.list" item="item"}
	<tr {if $key%2==1} class="tr2"{/if}>
		<td class="main_td1" align="center">{ $item.id}</td>
		<td class="main_td1" align="center">{$item.name}</td>
...
<div class="module_add">
	...
	<div class="module_border">
	<div class="l">费用比例：</div>
		<div class="c">
			<font color="#FF0000"><input type="text" name="fee"  value="{$_A.credit_rank_result.fee}" size="12"  /></font>%
		</div>
	</div>
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	<tr >
		...
		<td width="*" class="main_td">费用比例</td>
	...
	<tr {if $key%2==1} class="tr2"{/if}>
		...
		<td class="main_td1" align="center"> {$item.fee|default:0}%</td>
		<td class="main_td1" align="center"> {$item.nid}</td>
		<td class="main_td1" align="center"><a href="{$_A.query_url_all}&edit={$item.id}">修改</a> <a href="#" onClick="javascript:if(confirm('确定要删除吗?删除后将不可恢复')) location.href='{$_A.query_url_all}&del={$item.id}'">删除</a></td>
	</tr>
	{/loop}
...
<div class="module_add">
	...
	<div class="module_title"><strong>积分记录</strong><span style="float:right">
		用户名：<input type="text" name="name" id="username" value="{$magic.request.username|urldecode}"/>  标识名：<input type="text" name="nid" id="nid" value="{$magic.request.nid}" onkeyup="value=value.replace(/[^a-zA-Z_]/g,'')"/>   <input type="button" value="{$MsgInfo.users_name_sousuo}" / onclick="sousuo()"></span></div>
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	...
	{ list module="credit" function="GetLogList" var="loop" epage=20 username=request nid=request class_id=request user_id=request}
	{foreach from="$loop.list" item="item"}
	<tr {if $key%2==1} class="tr2"{/if}>
		<td class="main_td1" align="center">{ $item.id}</td>
		...
		<td class="main_td1" align="center"><font color="#FF0000" style="font-weight:bold">{if $item.credit==""}{$item.value}{else}{$item.credit}{/if}</font></td>
		<!-- <td class="main_td1" align="center"> {$item.remark|default:-}</td> -->
