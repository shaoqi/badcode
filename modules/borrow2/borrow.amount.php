...
class amountClass  extends autoClass {
	...
	function GetAmountApplyOne($data){
		...
		$sql = "select p1.*,p2.username,p3.fileurl as voucher_att_fileurl,p4.fileurl as vouchjg_xy_fileurl from `{borrow_amount_apply}` as  p1 
		left join `{users}` as p2 on p1.user_id=p2.user_id 
		left join {users_upfiles} as p3 on p1.voucher_att=p3.id
		left join {users_upfiles} as p4 on p1.vouchjg_xy=p4.id " . $_sql ." order by p1.id desc";
		$result = $mysql ->db_fetch_array($sql);		
		return $result;
	}
	...
	function GetAmountApplyList($data = array()){
		...
		if (IsExiest($data['group'])!=false) {
			$group="group by p1.type";
		}	
		$_order = " order by p1.id desc";
		$_select = 'p1.*,p2.username,p3.fileurl as voucher_att_fileurl,p4.fileurl as vouchjg_xy_fileurl';		
/* 		$sql = "select SELECT from {borrow_amount_apply} as p1 
				left join {users} as p2 on p1.user_id=p2.user_id
				left join {users_upfiles} as p3 on p1.voucher_att=p3.id
				left join {users_upfiles} as p4 on p1.vouchjg_xy=p4.id
				$_sql group by p1.type ORDER LIMIT ";
 */		
		$sql = "select SELECT from {borrow_amount_apply} as p1 
				left join {users} as p2 on p1.user_id=p2.user_id
				left join {users_upfiles} as p3 on p1.voucher_att=p3.id
	 ...
	 */
	 function GetAmountUsers($data = array()){
		...
		//$result['amounts'] = unserialize($result['amounts']);

		//$result["borrow"] = round($borrow_first+$borrow_credit+$result['borrow'],2);
		//$result["borrow_use"] = round($borrow_first+$borrow_credit+$result['borrow_use'],2);
		
		
		//add 20120828 wlz
		if($result["borrow_use"] < 0){
			$result["borrow_use"] = number_format(0,2);
		}
		//$result["borrow_nouse"] =$result["borrow_amount"] -$result["borrow_amount_use"];
