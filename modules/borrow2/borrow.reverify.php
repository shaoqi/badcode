...
class borrowreverifyClass
{
	...
	function ReverifyStep1($data){
		...
		if ($status == 3){
			...
			if ($borrow_frost_status ==1){
				...
				$log_info["to_userid"] = 0;//付给谁
				$log_info["remark"] =  "冻结借款标的[{$borrow_url}]的保证金";
				accountClass::AddLog($log_info);
			...
			if($borrow_result['borrow_type']!=4){			
				//借款成交费： （一次 借款人）	
				$manage_cjfee=isset($_G['system']['con_borrow_manage_cjfee'])?$_G['system']['con_borrow_manage_cjfee']:0.2;				
				$manage_month=isset($_G['system']['con_borrow_manage_month'])?$_G['system']['con_borrow_manage_month']:3;
				$manage_top=isset($_G['system']['con_borrow_manage_top'])?$_G['system']['con_borrow_manage_top']:0.4;
				$manage_top=isset($_G['system']['con_borrow_manage_dizen'])?$_G['system']['con_borrow_manage_dizen']:0.4;				
				if ($borrow_period<=$manage_month){
					$manage_fee = $borrow_account*$manage_cjfee/100;
				}elseif($borrow_period>$manage_month){				
					$manage_fee = $borrow_account*$manage_cjfee/100+$borrow_account*($borrow_period-$manage_month)*$con_borrow_manage_dizen/100;
				}
				if($manage_fee>$manage_top){$manage_fee=$manage_top/100;}
				
				$log_info["user_id"] = $borrow_userid;//操作用户id
				$log_info["nid"] = "borrow_fee_".$borrow_userid."_".$borrow_nid;//订单号
				...
				$log_info["frost"] = 0;//冻结金额
				$log_info["await"] = 0;//待收金额
				$log_info["type"] = "borrow_fee";//类型
				$log_info["to_userid"] = 0;//付给谁
				$log_info["remark"] = "用户借款成功[$borrow_url]扣除借款成交费";
				accountClass::AddLog($log_info);
				...
				//借款管理费		 wdf 20121011
				$borrow_manage=isset($_G['system']['con_borrow_manage_fee'])?$_G['system']['con_borrow_manage_fee']:0.003;
				$borrow_manage_fee = $borrow_account*$borrow_manage/100;
				$log_info["user_id"] = $borrow_userid;//操作用户id
				...
				$borrow_margin_fee = $borrow_account*$borrow_margin/100;
				$log_info["user_id"] = $borrow_userid;//操作用户id
				$log_info["nid"] = "borrow_margin_fee_".$borrow_userid."_".$borrow_nid;//订单号
				$log_info["money"] = $borrow_margin_fee;//操作金额
				$log_info["income"] = 0;//收入
				$log_info["expend"] = $borrow_margin_fee;//支出
				$log_info["balance_cash"] = -$borrow_margin_fee;//可提现金额
				$log_info["balance_frost"] = 0;//不可提现金额
				$log_info["frost"] = $borrow_margin_fee;//冻结金额
				$log_info["await"] = 0;//待收金额
				$log_info["type"] = "borrow_margin_fee";//类型
				$log_info["to_userid"] = 0;//付给谁
				$log_info["remark"] = "用户借款[$borrow_url]成功,冻结{$borrow_margin_fee}元保证金";
				accountClass::AddLog($log_info);
	...
	function ReverifyStep2($data){
		...
		if ($status == 3){
			...
			$remind['article_id'] = $borrow_nid;
			$remind['code'] = "borrow";
			$remind['title'] = "借款标[<font color=red>{$borrow_name}</font>]满标审核成功";
			$remind['content'] = "你发布的借款标[{$borrow_url}]在".date("Y-m-d",time())."满标审核通过";
			remindClass::sendRemind($remind);
			...
			$sql = "select * from `{borrow_tender}` as p1 where p1.borrow_nid='{$data['borrow_nid']}' limit {$data['key']},1";
			$tender_false = $mysql->db_fetch_array($sql);
			$tender_userid = $tender_false['user_id'];
			$tender_account = $tender_false['account'];	
			...
			$remind['article_id'] = $borrow_nid;
			$remind['receive_userid'] = $borrow_userid;
			$remind['title'] = "借款标[<font color=red>{$borrow_name}</font>]满标审核失败";
			$remind['content'] = "您所发布的借款标[{$borrow_url}]在".date("Y-m-d",time())."审核失败,失败原因：{$borrow_result['reverify_remark']}";
			remindClass::sendRemind($remind);
