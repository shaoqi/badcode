...
class borrowClass extends amountClass {
	...
	function Add($data = array()){
		...
		if($data['borrow_type']==1 || $data['borrow_type']==6){
			if ($data['account']>$result['borrow_use']){			
				return "borrow_account_over_credituse";
			}
		}elseif($data['borrow_type']==5){
			$result = amountClass::GetAmountUserWorth(array("user_id"=>$data["user_id"]));
			if ($data['account']>$result['all']){
				return "borrow_account_over_all";
			}
		}elseif($data['borrow_type']==2){
			if ($data['account']>$result['vouch_borrow_use']){			
				return "borrow_account_over_vouch";
			}
		}elseif($data['borrow_type']==3){
			if ($data['account']>$result['diya_borrow_use']){			
		...
		//判断金额的倍数
		if($data['account']%50!=0){
			return "borrow_account_beishu";
		}
		...
		$apr = isset($_G['system']['con_borrow_apr_max'])?$_G['system']['con_borrow_apr_max']:"22.18";
		...
		if( $data['borrow_type']==1 || $data['borrow_type']==6){		
			...
			$_data["account"] = $data['account'];
			$_data["remark"] = "担保借款[{$borrow_url}]审核通过扣去担保额度";//type 操作的类型 
			borrowClass::AddAmountLog($_data);
		}elseif( $data['borrow_type']==3 ){
			$_data["user_id"] = $data['user_id'];
			...
			$_data["type"] = "borrow_success";
			$_data["oprate"] = "frost";
			$_data["nid"] = "borrow_success_diya_".$borrow_userid."_".$data['borrow_nid']."_".$vouch_id;
			$_data["account"] = $data['account'];
...
/**
	...
	function GetList($data = array()){
		...
		if (IsExiest($data['query_type'])!=false){
			...
			elseif ($type=="vouch"){
				$_sql .= " and p1.borrow_type=5 and  p1.status=1";
			}
			//初审失败
			elseif ($type=="false"){
			...
			elseif ($type=="repay_yes"){
				$_sql .= " and p1.status=3 and (p1.repay_account_wait='0.00' or p1.borrow_type=4)";
			}
			
			elseif ($type=="repay_no"){
				$_sql .= " and p1.status=3 and p1.repay_account_wait!='0.00' and p1.borrow_type!=4";
			}
			//满标审核失败
		...
		if (IsExiest($data['index_status'])!=""){
			
			$_sql .= " and p1.status in ({$data['index_status']}) and p1.borrow_valid_time*60*60*24 + p1.verify_time >".time();
		...
		//排序初审 复审最新时间 add 20120828 wlz
		if($type=="invest"){
			$_order = " order by p1.`verify_time` desc ";
		}
		if($type=="full_check" || $type=="full_success"){
			$_order = " order by p1.`reverify_time` desc ";
		}
		...
		foreach ($list as $key => $value){
			...
			if($value['borrow_type'] == 4){
				//$list[$key]["borrow_period"] = $value['borrow_period']*30;
			}
	...
	public static function GetOne($data = array()){
		...
		$sql = "select p1.* ,p2.username,p3.username as verify_username from `{borrow}` as p1 
				...
				";
		$result = $mysql->db_fetch_array($sql);
		if($result['borrow_type']==6){
			$borrow_manage_fee_day= round($result['account']*0.0004,2);
			$borrow_each_day=round($result['account']*$result['borrow_apr']/100/365,2);
		}else{
			$borrow_each= round($result['account']/$result['borrow_period'],2);
			$borrow_manage_fee= round($result['account']*0.003,2);
			$borrow_manage_fee_total= round($result['account']*0.003*$result['borrow_period'],2);
			if ($result['borrow_period']<=3){
				$borrow_success_fee=0.02;
			}elseif($result['borrow_period']>3){
				$borrow_success_fee=($result['borrow_period']-3)*0.002+0.02;
			}
			if($borrow_success_fee>0.04){$borrow_success_fee=0.04;}		
			$success_fee = round($result['account']*$borrow_success_fee,2);	
		}		
		
		$result['borrow_each']=$borrow_each;
		$result['borrow_manage_fee']=$borrow_manage_fee;
		$result['borrow_manage_fee_day']=$borrow_manage_fee_day;
		$result['borrow_each_day']=$borrow_each_day;
		$result['borrow_manage_fee_total']=$borrow_manage_fee_total;
		$result['success_fee']=$success_fee;
	...
	function Verify($data = array()){
		...
		$sql = "select borrow_nid,status,borrow_type,`name`,borrow_valid_time,user_id,account from `{borrow}` where borrow_nid='{$data['borrow_nid']}'";
		$result = $mysql->db_fetch_array($sql);
		...
		$borrow_url = "<a href=/invest/a{$data['borrow_nid']}.html target=_blank style=color:blue>{$result['name']}</a>";//借款标地址
		...
		if ($data['status']==1){
			...
			//提醒设置 add 20120920 wlz
			$remind['nid'] = "borrow_yes";
			
			$remind['receive_userid'] = $result['user_id'];
			$remind['code'] = "borrow";
			$remind['article_id'] = $result['user_id'];
			$remind['title'] = "借款标[".$result['name']."]初审成功";
			$remind['content'] = "您的借款标[{$borrow_url}]在".date("Y-m-d",time())."初审通过";
			
			remindClass::sendRemind($remind);

		}elseif($status==2){
			
			if( $result['borrow_type']==1 || $result['borrow_type']==6){
				...
				$_data["account"] = $result['account'];
				$_data["remark"] = "担保借款[{$borrow_url}]初审不通过增加担保额度";//type 操作的类型 				
				borrowClass::AddAmountLog($_data);
				
			}elseif($result['borrow_type']==3 ){
				$_data["user_id"] = $result['user_id'];
				$_data["amount_type"] = "diya_borrow";
				$_data["type"] = "borrow_false";
				$_data["oprate"] = "return";
				$_data["nid"] = "borrow_false_diya_"."_".$result['borrow_nid']."_".$result['id'];
				$_data["account"] = $result['account'];
			...
			$remind['code'] = "borrow";
			$remind['article_id'] = $result['user_id'];
			$remind['title'] = "借款标[".$result['name']."]初审失败";
			$remind['content'] = "您的借款标[{$borrow_url}]初审没有通过";
			
			remindClass::sendRemind($remind);
		}
		
		//自动投标加入
		$res =autoClass::NewAutoTender(array("borrow_nid"=>$result['borrow_nid']));
		if ($res != false){
	...
	public static function Cancel($data = array()){
		...
		if( $borrow_result['borrow_type']==1 || $borrow_result['borrow_type']==6){		
			...
			borrowClass::AddAmountLog($_data);			
		}elseif($borrow_result['borrow_type']==2){
			$_data["user_id"] = $borrow_result['user_id'];
			$_data["amount_type"] = "vouch_borrow";
			$_data["type"] = "borrow_success";
			$_data["oprate"] = "return";
			...
			$_data["account"] = $borrow_result['account'];
			$_data["remark"] = "担保借款[{$borrow_url}]撤消返还担保额度";//type 操作的类型 
			borrowClass::AddAmountLog($_data);
		}elseif($borrow_result['borrow_type']==3){
			$_data["user_id"] = $borrow_result['user_id'];
			$_data["amount_type"] = "diya_borrow";
			$_data["type"] = "borrow_success";
			$_data["oprate"] = "return";
			$_data["nid"] = "borrow_return_diya_".$borrow_userid."_".$borrow_result['borrow_nid']."_".$vouch_id;
			$_data["account"] = $borrow_result['account'];
	...
	public static function AddTender($data = array()){
		...
		*/
		...
		$sql_a = "select username from `{users}` where user_id='{$data['user_id']}'";
		$result_a = $mysql->db_fetch_array($sql_a);
		$tender_username = $result_a['username'];
		
		//借款标投标成功站内信 add 20120920 wlz
		$remind['nid'] = "borrow_join";
		
		$remind['receive_userid'] = $borrow_result['user_id'];
		$remind['code'] = "borrow";
		$remind['article_id'] = $borrow_nid;
		$remind['title'] = $tender_username."投资了您的借款标";
		$remind['content'] = "您的借款标[{$borrow_url}]在".date("Y-m-d",time())."被".$tender_username."投资，投资金额￥".$data['account']."元。";
	...
	public static function Reverify($data = array()){
		...
		$borrow_url = "<a href=/invest/a{$data['borrow_nid']}.html target=_blank style=color:blue>{$borrow_result['name']}</a>";//借款标地址
		...
		if ($status == 3){
			...
			$credit=borrowClass::GetBorrowCredit(array("user_id"=>$borrow_userid));			//$rank=creditClass::GetUsersCreditRank(array("credit"=>$credit['credit_total']));
			$borrow_manage_fee = $rank['fee']*0.01;
			...
			$log_info["to_userid"] = 0;//付给谁
			$log_info["remark"] = "用户借款成功[$borrow_url]扣除信用管理费";
			//accountClass::AddLog($log_info);
			...
			if ($borrow_result['borrow_type']==4) {
				...
				$account_miao=$money+$borrow_account;
				$log_info["user_id"] = $borrow_userid;//操作用户id
				$log_info["nid"] = "borrow_repay_".$borrow_nid;//订单号
				$log_info["money"] = $account_miao;//操作金额
				$log_info["income"] = 0;//收入
				$log_info["expend"] = $account_miao;//支出
				$log_info["balance_cash"] = -$account_miao;//可提现金额
				$log_info["balance_frost"] = 0;//不可提现金额
				$log_info["frost"] = 0;//冻结金额
				$log_info["await"] = 0;//待收金额
				$log_info["repay"] = $repay_all;//待收金额
				...
				$log_info["to_userid"] = 0;//付给谁
				$log_info["remark"] =  "对借款标[{$borrow_url}]进行还款";
				accountClass::AddLog($log_info);
				
				$sql = "update `{borrow_repay}` set repay_status=1,repay_yestime='".time()."',repay_account_yes=repay_account,repay_interest_yes=repay_interest,repay_capital_yes=repay_capital where user_id='{$borrow_userid}' and borrow_nid='{$borrow_nid}'";				
				$mysql->db_query($sql);
				
				$sql = "update `{borrow}` set repay_account_yes=repay_account_wait,repay_account_interest_yes=repay_account_interest_wait,repay_account_capital_wait=repay_account_capital_wait where user_id='{$borrow_userid}' and borrow_nid='{$borrow_nid}'";				
				$mysql->db_query($sql);
			...
			*/
			...
			foreach ($tender_result as $_key => $_value){
				...
				$log_info["income"] = $recover_all;//收入
				$log_info["expend"] = 0;//支出
				$log_info["balance_cash"] = 0;//可提现金额
				$log_info["balance_frost"] = 0;//不可提现金额
				$log_info["frost"] = 0;//冻结金额
				...
				//秒标还款
				$log_info["user_id"] = $tender_userid;//操作用户id
				$log_info["nid"] = "tender_repay_".$borrow_nid.$tender_userid.$tender_id.$period_key;//订单号
				$log_info["money"] = $equal_result['account_total'];//操作金额
				$log_info["income"] = $equal_result['account_total'];//收入
				$log_info["expend"] = 0;//支出
				$log_info["balance_cash"] = $equal_result['account_total'];//可提现金额
				$log_info["balance_frost"] = 0;//不可提现金额
				$log_info["frost"] = 0;//冻结金额
				$log_info["await"] = -$equal_result['account_total'];//待收金额
				$log_info["type"] = "tender_repay_yes";//类型
				$log_info["to_userid"] = $borrow_userid;//付给谁
				$log_info["remark"] = "借款标[{$borrow_url}]还款成功金额增加";
				accountClass::AddLog($log_info);
				
				$recover_all = $equal_result['account_total'];
				$recover_interest_all = $equal_result['interest_total'];
				$recover_capital_all = $equal_result['capital_total'];
				$sql = "update  `{borrow_recover}` set recover_yestime='".time()."',recover_account_yes = recover_account ,recover_capital_yes = recover_capital ,recover_interest_yes = recover_interest ,status=1,recover_status=1 where user_id = '{$tender_userid}' and borrow_nid='{$borrow_nid}'";
				$mysql->db_query($sql);
				
				$sql = "update  `{borrow_tender}` set recover_times=recover_times+1,recover_account_yes= {$recover_all},recover_account_capital_yes =  {$recover_capital_all} ,recover_account_interest_yes = {$recover_interest_all},recover_account_wait= 0,recover_account_capital_wait = 0  ,recover_account_interest_wait = 0  where user_id = '{$tender_userid}' and borrow_nid='{$borrow_nid}'";
				$mysql->db_query($sql);
		...
		if ($borrow_result['borrow_type']==4 && $status==3){
	...
	function GetTenderBorrowList($data){
		...
		if (IsExiest($data['type'])!=""){
			if ($data['type']=="wait"){
				...
				//wdf 判断是否回收中的借款
				$_sql .= " and ((p1.recover_times<p2.borrow_period or p1.recover_account_all>p1.recover_account_yes) and p1.user_id={$user_id} and p1.recover_times!=p2.borrow_period and borrow_type!=4 ) or (borrow_type=4 and p1.change_status!=1 and p1.recover_account_all!=p1.recover_account_yes and p1.user_id={$user_id})";
				...
				//wdf 判断是否回收完
				$_sql .= " and ( p1.recover_times=p2.borrow_period and p1.user_id={$user_id}  and borrow_type!=4 ) or ( borrow_type=4  and p1.recover_account_all=p1.recover_account_yes and p1.user_id={$user_id})";
			}
		}else{
		...
		$_select  = "p2.*,p1.recover_times,p1.account as tender_account,p1.recover_account_yes,p1.user_id as tuser,p1.recover_account_all,p1.account_tender,p1.id as tid,p2.account as borrow_account,p2.borrow_account_yes,p3.username as borrow_username,p4.credits,p5.account as change_account,p5.id as change_id";
	...
	public static function BorrowRepay($data = array()){
		...
		if ($repay_period+1 == $borrow_period){
			if ($borrow_frost_account>0){
				...
				//保证金解冻站内信 add 20120919 wlz
				$remind['nid'] = "margin_thaw";
				
				$remind['receive_userid'] = $borrow_userid;
				$remind['code'] = "borrow";
				$remind['article_id'] = $borrow_nid;
				$remind['title'] = "保证金解冻成功";
				$remind['content'] = "您所还款的标[".$borrow_name."]在".date("Y-m-d",time())."的保证金已被解冻。";
		...
		*/
		...
		if ($repay_web!=1 && $repay_vouch!=1){
			...
			foreach ($result as $key => $value){
				...
				$remind['receive_userid'] = $value['user_id'];
				$remind['code'] = "invest";
				$remind['article_id'] = $value['user_id'];
				$remind['title'] = "客户（{$borrow_username}）对[{$borrow_name}]借款的还款";
	...
	public static function BorrowAdvanceRepay($data = array()){
		...
		foreach ($result as $key => $value){
			...
			$user_log["user_id"] = $value['user_id'];
			$user_log["code"] = "borrow";
			$user_log["type"] = "repay_success";
			$user_log["operating"] = "repay";
			$user_log["article_id"] = $borrow_userid;
			$user_log["result"] = 1;
			$user_log["content"] = "[{$borrow_url}]进行全额提前还款，收到还款";
			usersClass::AddUsersLog($user_log);	
		}
		...
		$credit_log['value'] = round($repay_capital*0.01,2);
		$credit_log['remark'] =  "借款[{$borrow_url}]还款所得积分";;
		creditClass::ActionCreditLog($credit_log);
		...
		$remind['title'] = "您已对[{$borrow_url}]成功还款。";
		$remind['content'] = "您已在".date("Y-m-d",time())."对[{$borrow_url}]全额提前还款成功";
		remindClass::sendRemind($remind);
		...
		$user_log["article_id"] = $borrow_userid;
		$user_log["result"] = 1;
		$user_log["content"] = "对借款标[{$borrow_url}]进行全额提前还款";
		usersClass::AddUsersLog($user_log);	
	...
	function GetBorrowRepayList($data = array()){
		...
		if (IsExiest($data['repay_type'])!=""){
			if ($data['repay_type']==1){
				$_sql .= " and p1.repay_status=1 and p1.late_days=0 and p1.advance_status=0";
			}elseif($data['repay_type']==2){
				$_sql .= " and p1.repay_status=1 and p1.late_days>0";
			}elseif($data['repay_type']==3){
				$_sql .= " and p1.repay_status=0 and p1.late_days=0";
			}elseif($data['repay_type']==4){
				$_sql .= " and p1.repay_status=0 and p1.late_days>0";
			}elseif($data['repay_type']==5){
				$_sql .= " and p1.repay_status=1 and p1.advance_status=1";
			}
		}
		if (IsExiest($data['status'])!=""){
			$_sql .= " and p1.status in ({$data['status']})";
		...
		if (isset($data['limit']) ){
			...
			foreach ($list as $key => $value){
				...
				if ($value['repay_status']!=1){
					if ($late['late_days']>0){
						$list[$key]['late_interest'] = round($value['repay_account']/100*0.4*$late['late_days'],2);
	...
	function GetUserCount($data){
		...
		$result=$mysql->db_fetch_arrays($sql);
		//提前还款统计   wdf  2012.9.17
		$advance_sql="select count(1) as advance_repay_times from `{borrow_repay}` where user_id={$data['user_id']} and repay_status=1 and repay_yestime<repay_time";
		$advance=$mysql->db_fetch_arrays($advance_sql);
		...
		$_result['advance_repay_times']=$advance[0]['advance_repay_times'];
		return $_result;
	}
