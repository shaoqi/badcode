...
$_A['list_purview']["borrow"]["result"]["borrow_late"] = array("name"=>"逾期借款","url"=>"code/borrow/late&late_days=0");
...
$_A['list_purview']["borrow"]["result"]["borrow_repay"] = array("name"=>"还款管理","url"=>"code/borrow/repay");
...
if ($_A['query_type'] == "first" ){
	...
	if ($_REQUEST['check']!=""){
		...
		if ($_POST['borrow_nid']){
			$var = array("name","borrow_use","borrow_period","borrow_style","borrow_apr","borrow_contents","borrow_day","borrow_type","borrow_valid_time","borrow_nid");
			$data = post_var($var);
			...
			if ($result>0){
				$msg = array("修改成功","",$_A['query_url']."/first");
			}else{
				$msg = array($MsgInfo[$result]);
			}
		}else{
	...
	if ($_REQUEST['fullcheck']!=""){
		...
		if ($borrow_result['borrow_type']==6){
			...
			if (isset($_POST['borrow_nid']) && $_POST['borrow_nid']!=""){
				...
				if ($result>0){
					$key = $data['key']+1;
					echo "第三步，正在审核投资信息，，请不要关闭浏览器。。。{$key}<script>location.href='{$_A['query_url_all']}&fullcheck={$_REQUEST['fullcheck']}&step=2&key={$key}'</script>";
				}else{
				...
				if ($result>0){
					$key = $_REQUEST['key']+1;
					echo "第四步，审核担保标信息，{$key}<script>location.href='{$_A['query_url_all']}&fullcheck={$_REQUEST['fullcheck']}&step=3&key={$key}'</script>";
				}else{
				...
				if ($result>0){
					$key = $_REQUEST['key']+1;
					echo "第五步，正在处理借款奖励信息，请不要关闭浏览器。。。{$key}<script>location.href='{$_A['query_url_all']}&fullcheck={$_REQUEST['fullcheck']}&step=4&key={$key}'</script>";
				}else{
 ...
 * 额度手工审核管理
**/
elseif ($_A['query_type'] == "amount_new" ){
	if ($_POST['type']!=""){
		$var = array("username","user_id","email","amount_type","type");
		$data = post_var($var);
		
		$data["limit"] = "all";
		$result = usersClass::GetUserid($data);
		if ($result>0){
			echo "<script>location.href='{$_A['query_url']}/amount_new&user_id={$result}&amount_type={$data['amount_type']}'</script>";
		}else{
	...
	elseif ($_REQUEST['user_id']!=""){
		...
		if (is_array($result)){
			$_A["amount_type_result"]['amount_type'] = $_REQUEST['amount_type'];
	...
	elseif (isset($_POST['oprate'])){
		...
		if ($msg==""){
			if(isset($_REQUEST['type']) && $_REQUEST['type'] == 2){
				$var = array("type","voucher_name","voucher_lianxi","voucher_address","amount_type","oprate","amount_account","content");
			}elseif(isset($_REQUEST['type']) && $_REQUEST['type'] == 3){
				$var = array("vouchjg_name","vouchjg_lianxi","vouchjg_address","amount_type","vouchjg_js","oprate","amount_account","content");
			}else{
			...
			if(isset($_REQUEST['type']) && $_REQUEST['type'] == 2){
				...
				if ($pic_result!=false){
					$data["voucher_att"] = $pic_result[0]["upfiles_id"];
				}
			}elseif(isset($_REQUEST['type']) && $_REQUEST['type'] == 3){
...
elseif ($_A['query_type'] == "repay_web"){
	...
	if ($_REQUEST['repay_id']=="" || $_REQUEST['borrow_nid']==""){
		...
		if ($_REQUEST["step"]==""){
			...
			if ($result>0){
				echo "第一步，判断是否可以审核。。。<script>location.href='{$_A['query_url']}/repay_web&repay_id={$_REQUEST['repay_id']}&borrow_nid={$_REQUEST['borrow_nid']}&user_id={$_REQUEST['user_id']}&step=1&key=0'</script>";
			}else{
				$msg = array($MsgInfo[$result]);
			}
		}elseif ($_REQUEST["step"]=="1"){
			...
			if ($result>0){
				$key = $_REQUEST['key']+1;
				echo "第二步，将本息还给投资人。。。<script>location.href='{$_A['query_url']}/repay_web&repay_id={$_REQUEST['repay_id']}&borrow_nid={$_REQUEST['borrow_nid']}&user_id={$_REQUEST['user_id']}&step=1&key={$key}'</script>";
			}elseif ($result==-1){
				echo "第三步，更新借款信息。。。<script>location.href='{$_A['query_url']}/repay_web&repay_id={$_REQUEST['repay_id']}&borrow_nid={$_REQUEST['borrow_nid']}&user_id={$_REQUEST['user_id']}&step=2'</script>";
			}else{
				$msg = array($MsgInfo[$result]);
			}
		}else{
