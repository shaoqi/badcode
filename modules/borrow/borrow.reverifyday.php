	...
	public static function ReverifyDay($data = array()){
		...
		$repaylixi=round($borrow_account*$borrow_day*$borrow_apr/100/365,2);
		 if ($status == 3){
			...
			$credit_log['value'] = round($borrow_account*0.001);
			...
			foreach ($tender_result as $_key => $_value){
				...
				if ($_value['status']!=1){
					...
					$remind['code'] = "borrow";
					$remind['title'] = "投资({$borrow_username})的标[{$borrow_name}]满标审核成功";
					...
					$credit_log['value'] = round($tender_account*0.001);
			...
			foreach ($tender_result as $key => $value){
				...
				$remind['title'] = "投资的标[{$borrow_name}]满标审核失败";
			...
			$remind['article_id'] = $borrow_nid;
			$remind['receive_user'] = $borrow_userid;
			$remind['title'] = "你所申请的标[{$borrow_name}]满标审核失败";
