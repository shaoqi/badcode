...
if(file_exists(ROOT_PATH."modules/borrow/borrow.".$_U['query_type'].".inc.php")){
    require_once(ROOT_PATH."modules/borrow/borrow.".$_U['query_type'].".inc.php");
}

//发布借款前是否可以检测
elseif ($_U['query_type'] == "loans"){	
	//借款类
...
//	}else
    ...
    elseif($_POST['account']<500 || $_POST['account']>1000000){
		...
		$var = array("name","borrow_use","borrow_period","borrow_style","account","borrow_apr","borrow_contents","borrow_type");
...
elseif ($_U['query_type'] == "gettender"){
...
elseif ($_U['query_type'] == "auto_new"){
	if ($_REQUEST['id']!=""){
		$data['user_id'] = $_G['user_id'];
		$data['id'] = $_REQUEST['id'];
		$_U['auto_result'] = borrowAutoClass::GetAutoOne($data);
	}
...
elseif ($_U['query_type'] == "auto_add"){
	...
	if ($data['tender_type']==2 && ($data['tender_scale']<1 || $data['tender_scale']>20)){
		...
		if (IsExiest($_POST['id']!="")){
			$data['id'] = $_POST['id'];
			$result = borrowAutoClass::UpdateAuto($data);
			$msg = array("自动投标信息修改成功","","/index.php?user&q=code/borrow/auto");
		}else{
			$result = borrowAutoClass::AddAuto($data);
			if ($result == -2){
...
elseif ($_U['query_type'] == "auto_list"){
...
elseif ($_U['query_type'] == "auto_del"){
	...
	$data['user_id'] = $_G['user_id'];
	$data["id"] = $_REQUEST['id'];
	$result = borrowAutoClass::DelAuto($data);
	if ($result!=1){
		$msg = array("你的操作有误，请不要乱操作");
	}else{
...
elseif ($_U['query_type'] == "add_care"){
	...
	if ($result==1){
		echo 1;exit;
		$msg = array("加入关注成功","","/?user&q=code/borrow/care");
	}else{
		echo 0;exit;
		$msg = array("已关注此标","","/?user&q=code/borrow/care");
...
//其他借款删除
elseif ($_U['query_type'] == "downpdf"){
	if($_REQUEST['nid']!=''){
		require_once(ROOT_PATH."modules/borrow/chinese/test.php");
		$borrow_nid = $_REQUEST['nid'];
		
		$result_head = borrowClass::GetView(array("borrow_nid"=>$borrow_nid));
		
		include_once("/chinese/test.php");
...
elseif ($_U['query_type'] == "xinbiao"){
	if ($_POST['name']){
		$var = array("account","apr_status","apr","borrow_type_status","borrow_type","borrow_period_status","borrow_period_start","borrow_period_end","account_status");
		$data = post_var($var);
		$data['status']=1;
		$data['user_id']=$_G['user_id'];
		$result=remindClass::AddRemindBorrow($data);
		if ($result===true){
			$msg = array("添加成功","","/?user&q=code/borrow/xinbiao");
		}else{
			$msg = array("添加失败","","/?user&q=code/borrow/xinbiao");
		}
	}elseif($_REQUEST['id']){		
		$data['id']=$_REQUEST['id'];
		$data['user_id']=$_G['user_id'];
		$result=remindClass::DeleteRemindBorrow($data);
		if ($result===true){
			$msg = array("操作成功","","/?user&q=code/borrow/xinbiao");
		}else{
			$msg = array("操作失败","","/?user&q=code/borrow/xinbiao");
		}
	}
}
//额度管理
elseif ($_U['query_type'] == "amount"){
	...
	if (isset($_POST['amount_type']) && $_POST['amount_type']!=""){
		...
		if ($result!=false && $result['addtime']+60*60*24*30 >time() && $result['status']==0){
			...
			if ($result>0){				
				if($_POST['borrowtype']!=''){
					$msg = array("您的申请已经被提交","","/?user&q=code/borrow/loan&p=approve");
				}else{
...
//正在招标的借款
elseif ($_U['query_type'] == "publish"){
}
elseif ($_U['query_type'] == "success"){
}
elseif ($_U['query_type'] == "lenddetail"){
}
elseif ($_U['query_type'] == "gathering"){
}
elseif ($_U['query_type'] == "care"){
}
elseif ($_U['query_type'] == "debting"){
}
elseif ($_U['query_type'] == "debt_move"){
}
elseif ($_U['query_type'] == "wait"){
}
elseif ($_U['query_type'] == "buy_success"){
}
elseif ($_U['query_type'] == "move_success"){
}
elseif ($_U['query_type'] == "auto"){
}
elseif ($_U['query_type'] == "yonghu"){
}
elseif ($_U['query_type'] == "xinbiao"){
}
elseif ($_U['query_type'] == "huikuan"){
}
elseif ($_U['query_type'] == "hetong"){
}
elseif ($_U['query_type'] == "jiangjin"){
}
elseif ($_U['query_type'] == "jrsh"){
}
elseif ($_U['query_type'] == "hkap"){
}


//正在还款的借款
elseif ($_U['query_type'] == "repayment"){
