...
function magic_function_monthpicker($data){
global $_G;
...
if ($data['value']{0}=="$"){
if ($date['value']!=""){
$result .= 'if ('.$data['value'].' >24){ '.$data['value'].' = date("H",'.$data['value'].');};';
}
$result .= 'for($i=0;$i<=24;$i++){ if ($i=='.$data['value'].'){ ';
}else{
if ($date['value']!=""){
$result .= 'if (\''.$data['value'].'\' >24){ \''.$data['value'].'\' = date("H",\''.$data['value'].'\');};';
}
$result .= 'for($i=0;$i<=24;$i++){ 
		if ($i==\''.$data['value'].'\'){';
}
