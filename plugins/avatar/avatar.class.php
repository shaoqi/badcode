...
class Avatar{
	...
	function uc_avatar($uid, $type = 'virtual', $returnhtml = 1) {
		...
		$uc_avatarflash = AVATAR_API.'/images/camera.swf?inajax=1&appid=1&input='.$uc_input.'&agent='.md5($_SERVER['HTTP_USER_AGENT']).'&ucapi='.urlencode(AVATAR_API."").'&avatartype='.$type;
		if($returnhtml) {
			return '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0" width="447" height="245" id="mycamera" align="middle">
				...
				<embed src="'.$uc_avatarflash.'" quality="high" bgcolor="#ffffff" width="447" height="245" name="mycamera" align="middle" allowScriptAccess="always" allowFullScreen="false" scale="exactfit"  wmode="transparent" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
			</object>';
		} else {
			return array(
				'width', '447',
	...
	function onrectavatar() {
		...
		//if(!is_dir(AVATAR_DATADIR.'./avatar/'.$home)) {
			//$this->set_home($uid, AVATAR_DATADIR.'./avatar/');
		//}
		$avatartype = $this->getgpc('avatartype', 'G') == 'real' ? 'real' : 'virtual';
		
		$bigavatarfile = AVATAR_DATADIR.'./avatar/'.$this->get_dw_avatar($uid, 'big', $avatartype);
		$middleavatarfile = AVATAR_DATADIR.'./avatar/'.$this->get_dw_avatar($uid, 'middle', $avatartype);
		$smallavatarfile = AVATAR_DATADIR.'./avatar/'.$this->get_dw_avatar($uid, 'small', $avatartype);
		...
		$this->DelPic(AVATAR_DATADIR.'./avatar/',$this->get_dw_avatar($uid, 'big', $avatartype));
		$this->DelPic(AVATAR_DATADIR.'./avatar/',$this->get_dw_avatar($uid, 'middle', $avatartype));
		$this->DelPic(AVATAR_DATADIR.'./avatar/',$this->get_dw_avatar($uid, 'small', $avatartype));
		...
		/*
		$uc_dir = "/dzx/uc_server";
		$home = $this->get_home($_SESSION['ucenter_id']);
		if(!is_dir(AVATAR_ROOT.$uc_dir.'/data/avatar/'.$home)) {
			$this->set_home($uid,AVATAR_ROOT.$uc_dir.'/data/avatar/');
		}
		copy($bigavatarfile, AVATAR_ROOT.$uc_dir.'/data/avatar/'.$this->get_avatar($_SESSION['ucenter_id'], 'big', $avatartype));
		copy($smallavatarfile, AVATAR_ROOT.$uc_dir.'/data/avatar/'.$this->get_avatar($_SESSION['ucenter_id'], 'small', $avatartype));  
		copy($middleavatarfile, AVATAR_ROOT.$uc_dir.'/data/avatar/'.$this->get_avatar($_SESSION['ucenter_id'], 'middle', $avatartype));    
		*/
		...
		$filetype = '.jpg';
		@unlink(AVATAR_DATADIR.'./tmp/upload'.$uid.$filetype);
	...
	function get_dw_avatar($uid, $size = 'big', $type = '') {
	...
	function avatar_show($uid, $size='small', $returnsrc = FALSE) {
		...
		return $returnsrc ? AVATAR_API.'/data/avatar/'.$avatarfile : '<img src="'.AVATAR_API.'/data/avatar/'.$avatarfile.'" onerror="this.onerror=null;this.src=\''.AVATAR_API.'/images/noavatar_'.$size.'.gif\'">';
	}
